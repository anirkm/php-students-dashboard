...
final class EntryParser
{
    ...
    {
        return self::splitStringIntoParts($entry)->flatMap(static function (
            array $parts
        ) {
            [$name, $value] = $parts;

            return self::parseName($name)->flatMap(static function (
                string $name
            ) use ($value) {
                $parsedValue =
                    $value === null
                ...
                return $parsedValue->map(static function (?Value $value) use (
                    $name
                ) {
    ...
    private static function parseValue(string $value)
    {
        ...
        return \array_reduce(
            ...
            static function (Result $data, string $token) {
                return $data->flatMap(static function (array $data) use (
                    $token
                ) {
                    return self::processToken($data[1], $token)->map(
                        static function (array $val) use ($data) {
                            return [
                                $data[0]->append($val[0], $val[1]),
                                $val[2],
                            ];
                        }
                    );
